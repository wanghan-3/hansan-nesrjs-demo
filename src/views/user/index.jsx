...
import { Card, Button, Table, message, Divider, Tag } from "antd";
import { getUsers, deleteUser, editUser, addUser } from "@/api/user";
import TypingCard from '@/components/TypingCard'
import EditUserForm from "./forms/edit-user-form"
import AddUserForm from "./forms/add-user-form"
import {getRolesAjax} from '@/api/auth'
const { Column } = Table;
      ...
      <div className="app-container">
        ...
        <Card title={title}>
          <Table bordered rowKey="id" dataSource={users} pagination={false}>
            ...
            <Column title="用户角色" dataIndex="roles" key="roles" align="center" render={(text, row)=>(
             text.map(m=><Tag color="geekblue" key={m.id}>{m.name}</Tag>)  
            )}/>
